...
// Event Data 3.0 is a 16 bits vectorized data format. It has been designed to
// comply with both data compactness necessity and vector event data support.
// This EVT3.0 event format avoids transmitting redundant event data for the
// time, y and x values.
enum class EventTypes : uint8_t {
  EVT_ADDR_Y = 0x0, // Identifies a CD event and its y coordinate
  EVT_ADDR_X = 0x2, // Marks a valid single event and identifies its polarity
                    // and X coordinate. The event's type and timestamp are
                    // considered to be the last ones sent
  VECT_BASE_X = 0x3, // Transmits the base address for a subsequent vector event
                     // and identifies its polarity and base X coordinate. This
                     // event does not represent a CD sensor event in itself and
                     // should not be processed as such,  it only sets the base
                     // x value for following VECT_12 and VECT_8 events.
  VECT_12 =
      0x4, // Vector event with 12 valid bits. This event encodes the validity
           // bits for events of the same type, timestamp and y coordinate as
           // previously sent events, while consecutive in x coordinate with
           // respect to the last sent VECT_BASE_X event. After processing this
           // event, the X position value on the receiver side should be
           // incremented by 12 with respect to the X position when the event
           // was received, so that the VECT_BASE_X is updated like follows:
           // VECT_BASE_X.x = VECT_BASE_X.x + 12
  VECT_8 =
      0x5, // Vector event with 8 valid bits. This event encodes the validity
           // bits for events of the same type, timestamp and y coordinate as
           // previously sent events, while consecutive in x coordinate with
           // respect to the last sent VECT_BASE_X event. After processing this
           // event, the X position value on the receiver side should be
           // incremented by 8 with respect to the X position when the event was
           // received, so that the VECT_BASE_X is updated like follows:
           // VECT_BASE_X.x = VECT_BASE_X.x + 8
  EVT_TIME_LOW =
      0x6, // Encodes the lower 12b of the timebase range (range 11 to 0). Note
           // that the TIME_LOW value is only monotonic for a same event source,
           // but can be non-monotonic when multiple event sources are
           // considered. They should however refer to the same TIME_HIGH value.
           // As the time low has 12b with a 1us resolution, it can encode time
           // values from 0us to 4095us (4095 = 2^12 - 1). After 4095us, the
           // time_low value wraps and returns to 0us, at which point the
           // TIME_HIGH value should be incremented.
  EVT_TIME_HIGH =
      0x8, // Encodes the higher portion of the timebase (range 23 to 12). Since
           // it encodes the 12 higher bits over the 24 used to encode a
           // timestamp, it has a resolution of 4096us (= 2^(24-12)) and it can
           // encode time values from 0us to 16777215us (= 16.777215s). After
           // 16773120us the time_high value wraps and returns to 0us.
...
struct Metadata {
  int sensor_width = 1280,
      sensor_height = 720; // Sensor width & height, by default assume PSEE Gen4
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    std::cerr << "Error : need input filename and output filename for CD events"
              << std::endl;
    std::cerr << std::endl
              << "Usage : " << std::string(argv[0])
              << " INPUT_FILENAME CD_OUTPUTFILE (TRIGGER_OUTPUTFILE)"
              << std::endl;
    std::cerr << "Trigger output file will be written only if given as input"
              << std::endl;
    std::cerr << std::endl
              << "Example : " << std::string(argv[0])
              << " input_file.raw cd_output.csv" << std::endl;
    std::cerr << std::endl;
    std::cerr << "The CD CSV file header will have the format : "
                 "%geometry:width,height"
              << std::endl;
    std::cerr << "The CD CSV file contents will have the format : "
                 "x,y,polarity,timestamp"
              << std::endl;
    std::cerr << "The Trigger CSV file contents will have the format : "
                 "value,id,timestamp"
              << std::endl;
    return 1;
  }
  ...
  if (argc > 3) {
    ...
    if (!trigger_output_file.is_open()) {
      std::cerr << "Error : could not open file '" << argv[3] << "' for writing"
                << std::endl;
      return 1;
    }
    write_triggers = true;
  ...
  while (input_file.peek() == '%') {
    ...
    if (header_line == "% end") {
      ...
      if (format_name != "EVT3") {
        std::cerr << "Error : detected non-EVT3 input file '" << argv[1] << "'"
                  << std::endl;
        return 1;
      }
      while (!sf.eof()) {
  ...
  // If parsed from input metadata, write sensor geometry as header of CD CSV
  // output
  if (metadata.sensor_width > 0 && metadata.sensor_height > 0) {
    cd_output_file << "%geometry:" << metadata.sensor_width << ","
                   << metadata.sensor_height << std::endl;
  }
  ...
  while (input_file) {
    ...
    Metavision::Evt3::RawEvent *last_word =
        current_word + input_file.gcount() / sizeof(Metavision::Evt3::RawEvent);

    // If the first event in the input file is not of type EVT_TIME_HIGH, then
    // the times of the first events might be wrong, because we don't have a
    // time base yet. This is why we skip the events until we find the first
    // time high, so that we can correctly set the current_time_base
    for (; !first_time_base_set && current_word != last_word; ++current_word) {
      Metavision::Evt3::EventTypes type =
          static_cast<Metavision::Evt3::EventTypes>(current_word->type);
      if (type == Metavision::Evt3::EventTypes::EVT_TIME_HIGH) {
        ...
        current_time_base =
            (Metavision::Evt3::timestamp_t(ev_timehigh->time) << 12);
    ...
    for (; current_word != last_word; ++current_word) {
      ...
      case Metavision::Evt3::EventTypes::EVT_ADDR_Y: {
        Metavision::Evt3::RawEventY *ev_addr_y =
            reinterpret_cast<Metavision::Evt3::RawEventY *>(current_word);
      ...
      case Metavision::Evt3::EventTypes::EVT_TIME_HIGH: {
        ...
        static constexpr Metavision::Evt3::timestamp_t MaxTimestampBase =
            ((Metavision::Evt3::timestamp_t(1) << 12) - 1)
            << 12; // = 16773120us
        static constexpr Metavision::Evt3::timestamp_t TimeLoop =
        ...
        Metavision::Evt3::RawEventTime *ev_timehigh =
            reinterpret_cast<Metavision::Evt3::RawEventTime *>(current_word);
        Metavision::Evt3::timestamp_t new_time_base =
            (Metavision::Evt3::timestamp_t(ev_timehigh->time) << 12);
      ...
      case Metavision::Evt3::EventTypes::EXT_TRIGGER: {
        if (write_triggers) {
          Metavision::Evt3::RawEventExtTrigger *ev_trigg =
              reinterpret_cast<Metavision::Evt3::RawEventExtTrigger *>(
                  current_word);
          ...
          // id = ev_trigg->id
          // time = current_time (in us)
          trigg_str += std::to_string(ev_trigg->value) + "," +
                       std::to_string(ev_trigg->id) + "," +
                       std::to_string(current_time) + "\n";
        }
        break;
      }
  ...
  const double duration_s =
      std::chrono::duration_cast<std::chrono::microseconds>(tp_end - tp_start)
          .count() /
      1e6;
  std::cout << "Decoded '" << argv[1] << "' in " << duration_s << " s"
            << std::endl;
